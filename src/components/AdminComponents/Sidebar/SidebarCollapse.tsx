...
const SidebarCollapse: React.FC<SidebarCollapseProps> = ({ imageSrc, label, items }) => {
    ...
    return (
        <div className='flex flex-col items-strech'>
            <div className='flex flex-1 gap-3 items-center sidebar-hover justify-between'>
                <div className='flex items-center gap-2'>
                {imageSrc && <img src={imageSrc} width={24} height={24}></img>}
                {label}
                </div>
                <img className={`align-middle items-center rotate-${rotation} transition ease-in-out duration-700`} src={ChevronUp} width={16} height={16} onClick={toggleCollapse} ></img>
            </div>
