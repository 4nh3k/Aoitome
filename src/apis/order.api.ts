...
import { AllDTO, AllOrdersDTO } from "@/types/Orders/AllOrdersDTO.type";
import { OrderDTO } from "@/types/Orders/OrderDto.type";

export const orderApi = {
  getAllOrders(pageNumber: number, pageSize: number) {
    return http.get<ApiResponse<AllDTO<OrderDTO>>>(`${ORDER_PREFIX}${GET_ALL_ORDER_URL}?pageNumber=${pageNumber}&pageSize=${pageSize}`);
  },
